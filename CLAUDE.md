...
- The road-geometry query should focus on intersection geometry and exclude the broad "Not at intersection" bucket. That bucket is large because there are many non-intersection road segments, so presenting it as the most dangerous road type is misleading.
- Keep unsupported-search fallbacks explicit. The static GitHub Pages version can answer council/year/severity/time/weekday/intersection questions, but should clearly decline live "near me", exact street/station, exposure-rate, or unavailable-column questions instead of pretending it knows.
